...
function bindPrivacyControls(){
  [$("privacyMessage"),$("privacyCall"),$("privacyRequests"),$("privacySearch")].filter(Boolean).forEach(el=>{
    el.addEventListener("change",()=>{
      ...
      save(); syncPrivacyDropdowns();
      window.a2lBackend?.saveProfile?.();
      window.a2lRealtime?.send({type:'profile-update',profile:{privacy:state.privacy}});
...
function go(page){
 ...
 if(page==="settings"){renderPrivacyControls();syncPrivacyDropdowns();}
...
if($("suggestedSearch"))$("suggestedSearch").oninput=()=>{
  renderPeople();
  const q=$("suggestedSearch").value.trim();
  if(q.length>=2){
    window.a2lBackend?.discoverUsers(q).then(users=>{
      let changed=false;
      for(const u of users||[]){
        if(!u?.a2lId||u.a2lId===state.profile.a2lId||friends.some(f=>f.a2lId===u.a2lId))continue;
        friends.push({
          id:Math.abs(u.a2lId.split('').reduce((h,c)=>(((h<<5)-h)+c.charCodeAt(0))|0,0))||999999,
...
renderProfile();renderHistory();bindPrivacyControls();renderPrivacyControls();
