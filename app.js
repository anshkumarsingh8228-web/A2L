...
function renderPrivacyControls(){
  ...
  if($("privacyQuickMatch"))$("privacyQuickMatch").checked=p.quickMatch!==false;
  if($("privacySearch"))$("privacySearch").checked=p.search!==false;
}
function bindPrivacyControls(){
  [$("privacyMessage"),$("privacyCall"),$("privacyRequests"),$("privacyQuickMatch"),$("privacySearch")].filter(Boolean).forEach(el=>{el.addEventListener("change",()=>{state.privacy={message:$("privacyMessage")?.value||"everyone",call:$("privacyCall")?.value||"everyone",requests:$("privacyRequests")?.checked!==false,quickMatch:$("privacyQuickMatch")?.checked!==false,search:$("privacySearch")?.checked!==false};save();toast("Privacy settings saved ✓")})});
...
function renderProfile(){
  ...
  const genBtn=$("generateA2LId");
  if(genBtn){
    genBtn.innerHTML=state.premium?'<span class="metallic-star profile-generate-star" aria-hidden="true">★</span> Generate new ID':'<span class="premium-lock-icon" aria-hidden="true">🔒</span> Generate new ID';
    genBtn.setAttribute("aria-disabled",String(!state.premium));
  }
...
$("generateA2LId").onclick=()=>{if(!state.premium){showPremiumGate();return;}$("profileIdInput").value=makeA2LId();};
