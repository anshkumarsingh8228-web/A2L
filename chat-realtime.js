...
(() => {
  ...
  function connect() {
    ...
    ws.addEventListener("open", () => {
      const sid=window.a2lIdentity?.getSessionId?.()||'';
      const profile=window.state?.profile||null;
      if(sid) ws.send(JSON.stringify({type:'register',sessionId:sid,name:profile?.displayName||'A2L user',profile:{a2lId:profile?.a2lId||'',name:profile?.displayName||'A2L user',avatar:profile?.avatar||'🙂',ageGroup:profile?.ageGroup||'',languages:profile?.languages||[],interests:profile?.interests||[],bio:profile?.bio||''}}));
      emit({type:"chat-socket-open"});
    });
