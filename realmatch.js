...
(function(){
  ...
  let muted=false, cameraOn=true, facing='user', liveFriendId=null, remoteVideoTrack=null, remoteAudio=null, connectionTimer=null, callSource='random', awaitingAccept=false, acceptPending=false;
  const EMOJIS=['❤️','😂','😮','👍','👏','🔥','🎉','😍'];
  ...
  function connect(){
    ...
    ws.onopen=()=>send({type:'register',sessionId,name:state.profile.displayName||'A2L user',profile:{name:state.profile.displayName||'A2L user',avatar:state.profile.avatar,ageGroup:state.profile.ageGroup,languages:state.profile.languages,interests:state.profile.interests,bio:state.profile.bio}});
  ...
  function ensureLiveFriend(id,name,profile){
    ...
    if(!f){
      f={id:fid,name:name||profile?.name||'A2L user',a2lId:id,avatar:profile?.avatar||'🙂',ageGroup:profile?.ageGroup||'16-17',languages:profile?.languages||['english'],online:true,interests:profile?.interests||[],bio:profile?.bio||'Live A2L connection'};
      friends.push(f);
    }else{f.name=name||profile?.name||f.name;f.a2lId=id;f.avatar=profile?.avatar||f.avatar;f.ageGroup=profile?.ageGroup||f.ageGroup;f.languages=profile?.languages||f.languages;f.interests=profile?.interests||f.interests;f.bio=profile?.bio||f.bio;f.online=true;}
  ...
  function show(kind){
    ...
    text('matchSessionTitle',kind==='video'?'Video Match':'Voice Match');text('matchSessionStatus',callSource==='friend-call'&&initiator?'Calling…':(initiator?'Finding a live user…':'Connecting…'));
    clearTimeout(connectionTimer);connectionTimer=setTimeout(()=>{if(pc?.connectionState!=='connected'){text('matchSessionStatus','Could not connect');end(true)}},30000);
  ...
  function setupPeer(){
    ...
    pc.onconnectionstatechange=()=>{
      ...
      if(s==='connected'){clearTimeout(connectionTimer);text('matchSessionStatus','Connected · LIVE');$('matchSessionModal')?.classList.add('is-connected');if(callSource==='random'){$('matchNext')?.toggleAttribute('hidden',false);$('voiceNext')?.toggleAttribute('hidden',false)}send({type:'match-state',to:peerId,busy:true});}
      if(s==='disconnected'){text('matchSessionStatus','Reconnecting…');}
      if(s==='failed'){text('matchSessionStatus','Connection failed · retrying…');try{pc.restartIce();}catch(_){}setTimeout(()=>{if(pc?.connectionState==='failed')end(true)},5000);}
  ...
  async function handle(m){
    ...
    if(m.type==='match-found'){peerId=m.peerId;peerName=m.peerName||m.peerProfile?.name||'A2L user';mode=m.mode||mode;initiator=!!m.initiator;start(mode,peerId,peerName,initiator,m.peerProfile||null,'random');return;}
  ...
  function startDirect(kind,id){
    ...
    connect();
    const invite=()=>send({type:'call-invite',to:peerId,kind:mode,source:'friend-call',profile:{name:state.profile.displayName||'A2L user',avatar:state.profile.avatar,ageGroup:state.profile.ageGroup,languages:state.profile.languages,interests:state.profile.interests,bio:state.profile.bio}});
    if(ws?.readyState===1)invite();else ws?.addEventListener('open',invite,{once:true});
    start(mode,peerId,peerName,true,f,'friend-call');
  }
  function find(kind){mode=kind||'video';connect();const run=()=>send({type:'find-match',mode,profile:{name:state.profile.displayName||'A2L user',avatar:state.profile.avatar,ageGroup:state.profile.ageGroup,languages:state.profile.languages,interests:state.profile.interests,bio:state.profile.bio}});if(ws?.readyState===1)run();else ws?.addEventListener('open',run,{once:true});show(mode);text('matchSessionStatus','Finding a live user…');}
  ...
  function end(notify=true){
    ...
    closeLiveChat();$('matchSessionModal')?.classList.remove('show');$('matchSessionModal')?.setAttribute('aria-hidden','true');
  }
  function init(){
    connect();
    ...
    window.a2lRealMatch={find,end,start,startDirect,nextMatch,sendChat,openLiveChat,getPeerId:()=>peerId,getLiveFriendId:()=>liveFriendId};
  }
