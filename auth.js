...
(() => {
  ...
  async function loadSession() {
    if (!window.supabase || !cfg.supabaseUrl || !cfg.supabaseAnonKey) {
      if (window.a2lSupabaseState?.ready) {
        cfg.supabaseUrl=window.a2lSupabaseState.url;
        cfg.supabaseAnonKey=window.a2lSupabaseState.key;
      } else {
        return {user:null, accessToken:null, configured:false};
      }
    }
    const client = window.a2lSupabase || window.supabase.createClient(cfg.supabaseUrl, cfg.supabaseAnonKey);
