...
(function(){
  ...
  function toggleSpeaker(){speakerOn=!speakerOn;const v=$('remoteMatchVideo');if(v)v.muted=!speakerOn;if(remoteAudio)remoteAudio.muted=!speakerOn;$('matchSpeaker')?.classList.toggle('off',!speakerOn);setText('matchSpeaker',speakerOn?'🔊':'🔇')}
  ...
  function switchMode(){const next=mode==='video'?'voice':'video';if(peer)send({type:'hangup',reason:'mode-switch'});cleanupPeer();peer=null;find({mode:next,language:'any',age:'any',interest:'any',online:true},source)}
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    $('matchReaction')?.addEventListener('click',e=>{e.stopPropagation();if($('reactionPicker'))$('reactionPicker').hidden=!$('reactionPicker').hidden});
    $('voiceReaction')?.addEventListener('click',e=>{e.stopPropagation();if($('reactionPicker'))$('reactionPicker').hidden=!$('reactionPicker').hidden});
    qsa('#reactionPicker button').forEach(b=>b.addEventListener('click',()=>react(b.textContent)));
    $('matchAddFriend')?.addEventListener('click',e=>{e.stopPropagation();addFriend()});
    $('matchNext')?.addEventListener('click',nextMatch);
    $('matchConnectBottom')?.addEventListener('click',nextMatch);
    ...
    document.addEventListener('click',e=>{const p=$('reactionPicker');if(p&&!p.hidden&&!e.target.closest('#reactionPicker')&&!e.target.closest('#matchReaction')&&!e.target.closest('#voiceReaction')){p.hidden=true}});
  });
