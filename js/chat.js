...
async function acceptChatRequest(requestId){
  ...
  r.status="accepted";r.acceptedAt=Date.now();save();
  if(id){state.chats[id]=state.chats[id]||{messages:[],source:"request",ended:false,locked:false,startedAt:Date.now()};state.chats[id].source="request";state.chats[id].ended=false;state.chats[id].locked=false;save();}
  renderChats();toast("Chat request accepted");if(id&&state.premium)openChat(id);
}
...
function openChat(id){
  ...
  window.a2lBackend?.conversation?.(f.a2lId).then(data=>{
    if(!data)return;
    c.messages=(data.messages||[]).map(m=>({serverId:m.id,text:m.body,mine:m.sender_id===state.profile.a2lId,at:new Date(m.created_at).getTime(),status:m.read_at?'read':m.delivered_at?'delivered':'sent'}));
...
function sendMessage(id){
 ...
 c.messages=c.messages||[];const msg={text,mine:true,at:Date.now(),status:'sent'};c.messages.push(msg);save();window.a2lBackend?.sendMessage((friends.find(f=>Number(f.id)===Number(id))?.a2lId)||String(id),text).then(serverMsg=>{msg.serverId=serverMsg.id;msg.status='delivered';save();renderMessages(id)}).catch(e=>console.warn('message persistence:',e.message));input.value="";
