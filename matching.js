...
function startQuickMatchDirect(){
  ...
  if(window.a2lCall?.startMatch){
    toast("Finding a live video match… ✨");
    window.a2lCall.startMatch("video",null,"quick");
    return;
  }
  toast("Live matching is reconnecting… please try again.");
}
...
function quickMatch(){
  ...
  const mode=selectedMode==="text"?"text":selectedMode;
  const prefs={mode,language:$('matchLanguage')?.value||"any",age:$('matchAge')?.value||"same",interest:$('matchInterest')?.value||"any",online:$('matchOnline')?.checked!==false};
  state.matchPrefs=prefs;save();
  if(mode==="text"){toast("Live text matching is not enabled yet.");return;}
  closeMatchModal();
  toast(`Finding a live ${mode} match… ✨`);
  window.a2lCall?.startMatch(mode,null,"vibe");
}
