...
async function api(req,res,body){
 ...
 if(req.method==='GET'&&url.pathname==='/api/health')return json(res,200,{ok:true,database:!!pool,auth:REQUIRE_AUTH&&!!SUPABASE_URL});
 ...
 if(!pool)return json(res,503,{error:'DATABASE_URL not configured'});if(parts[0]!=='api')return false;
 ...
 try{
  if(req.method==='PUT'&&parts[1]==='profile'){const p={...(body.profile||body)};delete p.a2lId;await ensureProfile({id,authUserId:uid},p);return json(res,200,{ok:true,a2lId:id});}
  if(req.method==='GET'&&parts[1]==='me'){const r=await q('select * from profiles where id=$1',[uid]);return json(res,200,r.rows[0]?publicPeerRow(r.rows[0]):null)}
  if(req.method==='GET'&&parts[1]==='profile'&&parts[2]){const r=await profileByA2L(parts[2]);return json(res,200,r.rows[0]?publicPeerRow(r.rows[0]):null)}
  ...
  if(req.method==='POST'&&parts[1]==='chat-response'){
    const r=await q('select * from chat_requests where id=$1 and receiver_id=$2',[body.requestId,uid]);if(!r.rows[0])return json(res,404,{error:'Request not found'});const x=r.rows[0];await q('update chat_requests set status=$1,updated_at=now() where id=$2',[body.accepted?'accepted':'declined',body.requestId]);const from=await q('select a2l_id from profiles where id=$1',[x.sender_id]);await notify(from.rows[0]?.a2l_id,body.accepted?'chat_accepted':'chat_declined',id,{requestId:body.requestId});return json(res,200,{ok:true});}
  if(req.method==='GET'&&parts[1]==='history'){
    const r=await q(`select h.*,case when h.user_a=$1 then h.user_b else h.user_a end as other_id,p.a2l_id as other_a2l_id,p.display_name,p.avatar_url,p.photo_data from connection_history h join profiles p on p.id=case when h.user_a=$1 then h.user_b else h.user_a end where h.user_a=$1 or h.user_b=$1 order by h.last_seen_at desc limit 100`,[uid]);return json(res,200,r.rows);}
  if(req.method==='POST'&&parts[1]==='reconnect'){
    const t=await profileByA2L(body.to),to=t.rows[0]?.id;if(!to||to===uid)return json(res,400,{error:'Invalid user'});const valid=await q('select 1 from connection_history where id=$1 and reconnectable=true and (user_a=$2 or user_b=$2) and (case when user_a=$2 then user_b else user_a end)=$3',[body.historyId,uid,to]);if(!valid.rows[0])return json(res,400,{error:'Reconnect history is invalid'});
    const r=await q(`insert into reconnect_requests(from_user,to_user,history_id) values($1,$2,$3) returning *`,[uid,to,body.historyId]);await notify(body.to,'reconnect_request',id,{requestId:r.rows[0].id,historyId:body.historyId});return json(res,201,r.rows[0]);}
