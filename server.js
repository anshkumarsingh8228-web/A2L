...
async function api(req,res,body){
 const url=new URL(req.url,`http://${req.headers.host}`),parts=url.pathname.split('/').filter(Boolean);
   if(req.method==='GET'&&url.pathname==='/api/health'){let dbOk=false,dbErr=null;try{if(pool){await pool.query('select 1');dbOk=true;}}catch(e){dbErr=e.message;}const dbHost=RESOLVED_DATABASE_URL?(RESOLVED_DATABASE_URL.match(/@([^/:]+)/)||[])[1]:null;const dbUser=RESOLVED_DATABASE_URL?(RESOLVED_DATABASE_URL.match(/:\/\/([^:]+)/)||[])[1]:null;return json(res,200,{ok:true,database:dbOk,dbError:dbErr,dbHost,dbUser,auth:REQUIRE_AUTH&&!!SUPABASE_URL});}
