...
function safeProfile(p,id,name){p=p||{};const handle=String(p.a2lId||p.handle||'').trim().slice(0,30);return {userId:id,a2lId:handle||id,name:String(p.name||name||'A2L user').slice(0,80),avatar:String(p.avatar||'🙂').slice(0,12),ageGroup:String(p.ageGroup||'').slice(0,20),languages:Array.isArray(p.languages)?p.languages.slice(0,5):[],interests:Array.isArray(p.interests)?p.interests.slice(0,10):[],bio:String(p.bio||'').slice(0,240),status:String(p.status||'Available to chat').slice(0,80)};}
function publicUser(ws){return {...ws.profile,userId:ws.sessionId,online:isLive(ws),inCall:!!ws.busy};}
function publicKnown(id){const ws=peers.get(id), profile=profiles.get(id)||ws?.profile||safeProfile({},id,'A2L user');return {...profile,userId:id,online:isLive(ws),inCall:!!ws?.busy};}
function peerByPublicId(value){const key=String(value||'');if(peers.has(key))return peers.get(key);for(const ws of peers.values()){if(ws.profile?.a2lId===key)return ws;}return null;}
...
function queueMatch(ws,mode,name,profile){
  mode=['video','voice','text'].includes(mode)?mode:'video';removeFromQueue(ws);
  const candidate=waiting.get(mode);
  if(candidate&&isLive(candidate.ws)&&candidate.ws.sessionId!==ws.sessionId){
    clearTimeout(candidate.timer);waiting.delete(mode);
    candidate.ws.peerId=ws.sessionId;ws.peerId=candidate.ws.sessionId;
    const common={type:'match-found',mode};
    send(candidate.ws,{...common,peerId:ws.sessionId,peerName:name||'A2L user',peerProfile:profile||null,initiator:true});
    send(ws,{...common,peerId:candidate.ws.sessionId,peerName:candidate.name||'A2L user',peerProfile:candidate.profile||null,initiator:false});
    return;
  }
  waiting.set(mode,{ws,name:name||'A2L user',profile:profile||null,timer:null,queuedAt:Date.now()});
  send(ws,{type:'waiting',mode});
}
function sendFriendRequest(ws,m){
  const rawTo=String(m.to||'');const target=peerByPublicId(rawTo);const to=target?.sessionId||rawTo;if(!to||to===ws.sessionId||!peers.has(to))return;
...
wss.on('connection',ws=>{
  ...
  ws.on('message',raw=>{
    ...
    if(m.type==='chat-message' && m.to){
      const target=String(m.to);
      const text=String(m.text||'').trim().slice(0,2000);
      if(!text)return;
      const recipient=peers.get(target)||peerByPublicId(target);
      if(!recipient || !isLive(recipient) || isBlocked(sessionId,recipient.sessionId) || isBlocked(recipient.sessionId,sessionId))return;
      const message={id:String(m.clientMessageId||`${sessionId}-${Date.now()}`),from:sessionId,fromName:ws.name,peerProfile:ws.profile,text,at:Date.now()};
      send(recipient,{type:'chat-message',message,from:sessionId,fromName:ws.name,peerProfile:ws.profile,text:message.text,at:message.at});
      send(ws,{type:'chat-delivered',clientMessageId:message.id,to:recipient.sessionId});
      return;
    }
    ...
    if(m.type==='hangup'){
      const targetId=String(m.to||ws.peerId||'');
      ws.busy=false;ws.peerId=null;
      if(targetId){const target=peers.get(targetId);if(target){target.busy=false;target.peerId=null;}route(targetId,{...m,from:sessionId,fromName:ws.name});}
      sendPresence();return;
    }
  ...
  ws.on('close',()=>{
    removeFromQueue(ws);
    const targetId=ws.peerId;
    if(targetId){const target=peers.get(targetId);if(target){target.busy=false;target.peerId=null;send(target,{type:'peer-disconnected',from:sessionId,fromName:ws.name});}}
    ws.peerId=null;
    if(sessionId&&peers.get(sessionId)===ws){peers.delete(sessionId);sendPresence();}
  });
});
