...
(function(){
  ...
  async function startMatch(kind,id,src){
    ...
    try{
      ...
      // Wait for the callee to accept before sending the WebRTC offer.
      setText('matchSessionStatus',"Ringing…");
  ...
  function bindSocketHandlers(){
    const socket=window.a2lSocket;
    if(!socket || socketHandlersBound) return;
    socketHandlersBound=true;
    socket.on("call:invite",onCallInvite);
    socket.on("call:busy",()=>setText("matchSessionStatus","User is busy"));
    socket.on("call:reject",()=>setText("matchSessionStatus","Call declined"));
    socket.on("call:accepted",async()=>{
      setText("matchSessionStatus","Connecting…");
      // The callee has now created its RTCPeerConnection.
      if(initiator && pc && callPeerA2LId){
        try{
          await createOffer();
  ...
  // realtime.js creates Socket.IO asynchronously after DOMContentLoaded.
  window.addEventListener("a2l:connected",bindSocketHandlers);
