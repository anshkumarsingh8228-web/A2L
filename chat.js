...
function openChat(id){
  ...
  <div class="chatbox"><div class="messages" id="messages"></div><div class="chat-warning" id="chatWarning" role="alert"></div><div class="chat-quick-tools"><button class="secondary a2l-state-button" id="chatLikePerson" type="button">❤️ Like</button><button class="secondary" id="chatPlayGame" type="button">🎮 Play</button><button class="secondary chat-more-btn" id="chatMoreActions" type="button">More</button></div><div class="composer"><div class="composer-input-wrap"><input id="messageInput" maxlength="500" placeholder="Write a message…" autocomplete="off"><button class="composer-emoji-btn" id="composerEmoji" type="button" aria-label="Open emoji reactions">😊</button><div class="composer-emoji-picker" id="composerEmojiPicker" hidden></div></div><button class="primary" id="sendMessage" type="button">Send</button></div></div>`;
  ...
  if(reactionPicker&&reactionButton){
    ...
    qsa(".composer-emoji-option").forEach(b=>{b.addEventListener("pointerdown",e=>e.preventDefault());b.onclick=()=>{showChatReaction(b.textContent);reactionPicker.hidden=true;$("messageInput")?.focus({preventScroll:true});};});
  }
  $("chatLikePerson").onclick=()=>likePerson(id);
...
const CHAT_WALLPAPERS=[
  {id:"midnight",name:"Midnight",className:"wp-midnight"},
  {id:"purple",name:"A2L Purple",className:"wp-purple"},
  {id:"nebula",name:"Nebula",className:"wp-nebula"},
  {id:"aurora",name:"Aurora",className:"wp-aurora"},
  {id:"minimal",name:"Minimal",className:"wp-minimal"}
];
...
function applyChatWallpaper(id, wallpaperId){
  ...
  const selected=wallpaperId||state.chatWallpapers?.[id]||"midnight";
  box.classList.add("wallpaper-"+selected);
}
function renderWallpaperOptions(id){
  const grid=$("wallpaperGrid"); if(!grid)return;
  const selected=pendingChatWallpaperId||state.chatWallpapers?.[id]||"midnight";
...
function openWallpaperPicker(id){
  pendingChatWallpaperOriginal=state.chatWallpapers?.[id]||"midnight";
...
function closeWallpaperPicker(id,saveChoice=false){
  if(saveChoice){
    const chosen=pendingChatWallpaperId||pendingChatWallpaperOriginal||"midnight";
    state.chatWallpapers[id]=chosen;
    save();
    applyChatWallpaper(id,chosen);
    toast("Wallpaper applied ✓");
  }else{
    const original=pendingChatWallpaperOriginal||state.chatWallpapers?.[id]||"midnight";
...
function sendMessage(id){
 ...
 c.messages=c.messages||[];const msg={text,mine:true,at:Date.now(),status:'sent'};c.messages.push(msg);save();input.value="";
 if(warning){warning.textContent="";warning.classList.remove("show");}
 renderMessages(id);
 setTimeout(()=>{if(!state.chats[id]||!state.chats[id].messages.includes(msg))return;msg.status='delivered';save();renderMessages(id);},450);
