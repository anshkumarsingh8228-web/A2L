...
function chatAccess(c){
  ...
  if(c.source==='quick'||c.source==='friend'||c.source==='stranger'||c.source==='request') return {allowed:true,reason:'live-or-friend'};
...
function initChatCategoryControls(){
  ...
  $("startStrangerChat")?.addEventListener("click",()=>{
    if(typeof window.a2lRealMatch?.find==='function'){ toast('Finding a stranger… 💬'); window.a2lRealMatch.find('text'); }
    else toast('Live chat is reconnecting…');
  });
}
...
function acceptChatRequest(requestId){
  ...
  if(!r)return;
  const id=Number(r.fromId||r.fromUserId||r.toId||0);
  if(!id){r.status="accepted";save();renderChats();return;}
  state.chats[id]=state.chats[id]||{messages:[],source:"request",ended:false,locked:false,startedAt:Date.now()};
  state.chats[id].source="request";state.chats[id].ended=false;state.chats[id].locked=false;
  r.status="accepted";r.acceptedAt=Date.now();
  save();renderChats();
  toast("Chat request accepted");
  if(state.premium)openChat(id);
}
...
function renderChats(){
  ...
  const strangerBtn=$("startStrangerChat"); if(strangerBtn) strangerBtn.style.display=chatCategory==="strangers"?"block":"none";
...
function requestConnection(id){const f=friends.find(x=>x.id===Number(id));if(!f)return;if(f.isFriend){toast(`${f.name} is already your friend 🤝`);return}if(window.a2lSocial?.request){window.a2lSocial.request(f,'profile');return;}toast("Friend service is reconnecting…")}
function respondConnection(accepted){const reqId=window.__incomingConnectionId,req=state.connectionRequests?.[reqId];if(!req)return;closeModal("connectionRequestModal");if(window.a2lSocial){accepted?window.a2lSocial.accept(req):window.a2lSocial.decline(req);req.status=accepted?"accepted":"declined";save();if(!accepted)toast("Connection request declined");}else toast("Friend service is reconnecting…")}
...
function blockPerson(id){
  ...
  if(!confirm(`Block ${f.name}? They will disappear from discovery.`))return;
  if(!state.blocked.includes(id))state.blocked.push(id);
  delete state.chats[id];state.connections=(state.connections||[]).filter(x=>x!==id);save();
  activeChatId=null;activeChatProfile=false;go('home');renderPeople();toast(`${f.name} blocked 🛡️`);
}
function reportPerson(id){
  const f=friends.find(x=>x.id===id);if(!f)return;
  const reason=prompt(`Report ${f.name}. What is the reason?`,"Inappropriate behaviour");
  if(reason===null)return;
  toast("Report submitted for review 🛡️");
}
function openProfile(id){
  ...
  $('chatStage').innerHTML=`<div class="card"><div class="profileHead profileHead-chat"><button class="profile-back-btn" id="profileBackBtn" type="button" aria-label="Back">‹</button><div class="profileHead-main"><div class="bigavatar">${String(f.avatar||"🙂").startsWith("http")?`<img src="${escapeHTML(f.avatar)}" alt="${escapeHTML(f.name)}">`:escapeHTML(f.avatar||"🙂")}</div><div><h2 style="margin:0">${escapeHTML(f.name)}</h2><p class="muted" style="margin:5px 0 0">${f.online?"🟢 Online":"⚪ Offline"}</p><p class="a2l-profile-id-prominent">@${escapeHTML(f.a2lId||"a2l_user")}</p></div></div><button class="chat-unified profile-inline-chat" id="profileChatBtn"><span>Chat</span></button></div><div class="a2l-profile-actions"><button class="secondary a2l-like-display" id="profileLikeBtn" type="button" aria-label="Like ${escapeHTML(f.name)}">❤️ ${likes.toLocaleString()} likes</button><button class="secondary a2l-connect-btn ${connected?"connected":""}" id="profileConnectBtn" type="button">${connected?"🤝 Connected":state.outgoingConnectionRequests?.[f.userId||f.a2lId]?.status==="pending"?"Friend request sent":"＋ Add Friend"}</button></div><div class="a2l-safety-row"><button class="secondary" id="profileReportBtn">⚑ Report</button><button class="danger" id="profileBlockBtn">⛔ Block</button></div></div>
  <div class="card"><h3 style="margin-top:0">About ${escapeHTML(f.name)}</h3><p class="a2l-profile-id-line"><b>A2L ID</b><span>@${escapeHTML(f.a2lId||"a2l_user")}</span></p><p class="muted">${escapeHTML(f.bio)}</p><div class="chips">${f.interests.map(x=>`<span class="chip">${escapeHTML(x)}</span>`).join("")}</div><div class="profile-info-compact"><div><span class="profile-info-label">Language</span><strong>${f.languages?.includes("english")&&f.languages?.includes("hindi")?"English + Hindi":f.languages?.includes("english")?"English":"Hindi"}</strong></div><div><span class="profile-info-label">Age group</span><strong>${escapeHTML(f.ageGroup||"—")}</strong></div></div></div>`;
  go("chat");
  ...
  $('profileChatBtn').onclick=()=>{let current=state.chats[id]||null;if(state.premium){current=current||{messages:[],source:"suggested",ended:false,locked:false,startedAt:Date.now()};current.source="suggested";if(current.ended){toast("This chat has ended. Start a new chat from Suggested Chats.");return;}current.locked=false;state.chats[id]=current;save();openChat(id);return;}if(current&&current.ended!==true&&['quick','friend','stranger','request'].includes(current.source)){current.locked=false;save();openChat(id);return;}if(chatAccess(current).allowed){openChat(id);return;}showPremiumGate();};
}
...
function openChat(id){
  ...
  $("chatStage").innerHTML=`<div class="chat-wa-header">
    ...
    <div class="chat-wa-avatar chat-header-profile-target" data-chat-header-profile="1" role="button" tabindex="0" title="Open profile" aria-label="Open profile">${String(f.avatar_url||f.avatar||"🙂").startsWith("http")?`<img src="${escapeHTML(f.avatar_url||f.avatar)}" alt="${escapeHTML(f.name)}">`:escapeHTML(f.avatar||"🙂")}</div>
  ...
  if(window.a2lData?.isReady?.() && f.userId){
    const targetUser=f.authUserId||f.userId;
    window.a2lData.ensureConversation(targetUser,c.source==='stranger'?'stranger':'direct').then(async cid=>{
      if(!cid)return;
      c.conversationId=cid;c.dbUserId=targetUser;c.livePeerId=targetUser;save();
      const rows=await window.a2lData.loadMessages(cid);
      if(rows.length){
        c.messages=rows.map(m=>({text:m.body,mine:String(m.sender_id)===String(window.a2lData.userId),at:new Date(m.created_at).getTime(),status:String(m.sender_id)===String(window.a2lData.userId)?'read':'received',dbId:m.id}));
        save();renderMessages(id);
      }
      window.a2lChat?.history?.(cid);
    });
  }
...
function sendMessage(id){
 ...
 c.messages=c.messages||[];const msg={text,mine:true,at:Date.now(),status:'sent',clientMessageId:'cm_'+Date.now()+'_'+Math.random().toString(36).slice(2)};c.messages.push(msg);save();input.value="";
 // Live-match chats are routed through the real WebSocket peer instead of the old local-only demo bus.
 if(c.conversationId && c.livePeerId) window.a2lChat?.send?.(c.conversationId,text,msg.clientMessageId);
 if(c.conversationId && window.a2lData?.persistMessage){
   window.a2lData.persistMessage(c.conversationId,text,msg.clientMessageId).then(r=>{if(r?.error)toast('Live message sent; cloud save failed')});
 }
