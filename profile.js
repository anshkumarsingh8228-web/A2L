...
async function applyCrop(){
  const st=cropImageState;if(!st.img)return;const source=$("profileCropCanvas"),out=document.createElement("canvas"),size=512;out.width=out.height=size;const ctx=out.getContext("2d");ctx.drawImage(source,0,0,size,size);const dataUrl=out.toDataURL("image/jpeg",.88);state.profile.photoData=dataUrl;save();closeCropper();renderProfile();
  const synced=await window.a2lData?.uploadAvatar?.(dataUrl);
  toast(synced?.error?"Photo updated locally; cloud sync failed":"Profile photo updated & synced ✓");
}
